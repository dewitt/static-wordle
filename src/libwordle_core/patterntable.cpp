...
static uint8_t calc_pattern_fast(const PackedWord& guess, const PackedWord& secret) {
...
void PatternTable::generate(const std::vector<std::string>& guesses, const std::vector<std::string>& solutions) {
    ...
    for(size_t i=0; i<num_guesses_; ++i) packed_guesses[i] = pack(guesses[i]);
    for(size_t i=0; i<num_solutions_; ++i) packed_solutions[i] = pack(solutions[i]);
        ...
        futures.push_back(std::async(std::launch::async, [this, start, end, &packed_guesses, &packed_solutions]() {
            for (size_t g = start; g < end; ++g) {
                ...
                for (size_t s = 0; s < num_solutions_; ++s) {
                    table_[row_offset + s] = calc_pattern_fast(guess_word, packed_solutions[s]);
                }
            }
        }));
    }
